...
pub async fn start_staking(
    ...
    // The hot wallet holds a single key. Staking to a foreign address is
    // allowed (delegate-style flows and the integration suite rely on it),
    // but kernels our signatures can't satisfy will be rejected by the
    // chain — flag it so the caller knows.
    let owned = matches!(
        state.wallet_change_address.read().await.clone(),
        Some(addr) if addr == req.address
    );
    ...
    // Standalone/test mode wires no engine: record the intent; a dead
    // engine with a live channel errors instead of lying.
    if let Some(tx) = &state.staking_control {
        ...
        .await
        .map_err(|_| RpcError::Internal("Staking engine unavailable — restart the node".into()))?;
    }
    ...
    *state.staking_address.write().await = Some(req.address.clone());
    ...
    Ok(Json(json!({
        ...
        "owned": owned,
        "warning": if owned { None } else { Some("Staking address is not owned by the hot wallet — kernels it cannot sign will be rejected by the chain") },
    })))
}
